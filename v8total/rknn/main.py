...
class YOLOV8RKNN:
    ...
    def __init__(self, modelpath, conf_thres=0.7, nms_thres=0.8):
        ...
        self.rknn.load_rknn(modelpath)
        self.rknn.init_runtime()
        # self.rknn.init_runtime(target='rk3568', device_id="f331b451282d351c")

    def drawPred(self, image, box, conf, clsid):
        x,y,w,h = box

        cv2.rectangle(image, (x,y), (x+w,y+h), (90,123,45), 2)

        label = self.class_names[clsid] + ":" + str(round(conf * 100)) + "%"

        cv2.putText(image, label, (x,y-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (123,0,234), 2)
    ...
    def resize_img(self, image):
        ...
        if srch != srcw:
            ...
            simg = cv2.resize(image, (neww, newh), cv2.INER_AREA)
    ...
    def detect(self, image):
        ...
        for pred in np.squeeze(outputs[0]).transpose():
...
if __name__ == "__main__":
    ...
    net = YOLOV8RKNN("weights/yolov8n.rknn")

    srcimg = cv2.imread("imgs/bus.jpg")

    tarimg = net.detect(srcimg)

    cv2.imwrite("imgs/bus_rk.jpg", tarimg)
